...
# 	if np.mean(temp) <= strictMean:
# 		checkOneStrict.append(pic)
# 	elif strictMean < np.mean(temp) <= softMean:
...
# 	if all(i >= failCorr for i in temp):
# 	# if temp >= failCorr:
# 		checkTwoPass.append(pic)
# 	else:
# 		checkTwoFail.append(pic)
...
# # 	axarr[1].imshow(image)
# # 	plt.title("Pass")
# # 	plt.savefig('Out/'+img[1][0:8]+'_SoftPass.png')
# # 	plt.close("all")
...
# # 	axarr[1].imshow(image)
# # 	plt.title("Fail")
# # 	plt.savefig('Out/'+img[1][0:8]+'_Fail.png')
# # 	plt.close("all")

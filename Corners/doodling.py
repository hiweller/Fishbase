...
for pic in checkOneFail:
	...
	# if temp >= failCorr:
		checkTwoPass.append(pic)
	elif np.mean(ctrcorr) <= softCtr and max(ctrcorr) <= maxCtr:
		checkTwoPass.append(pic)
	else:
		checkTwoFail.append(pic)
...
print 'checkOneStrict'
for img in checkOneStrict:
	pic = img[0]
	print img[1], img[2]
	f, axarr = plt.subplots(2)
	imname = args["dataset"]+img[1]
	image = cv2.imread(imname)
	image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
	# axarr[0].plot(pic[0], 'y-', pic[1], 'g-', pic[2], 'b-', pic[3], 'r-', pic[4], 'm^-')
	axarr[0].plot(pic[0], 'y-', pic[1], 'g-', pic[2], 'b-', pic[3], 'r-')
	axarr[1].imshow(image)
	# plt.imshow(image)
	plt.title("Pass")
	plt.savefig('./'+img[1][0:(len(img[1])-4)]+'_StrictPassCorner.png')
	plt.close("all")
	# plt.show()
...
for img in checkTwoPass:
	...
	axarr[1].imshow(image)
	plt.title("Pass")
	plt.savefig('./'+img[1][0:(len(img[1])-4)]+'_SoftPassCorner.png')
	plt.close("all")
...
for img in checkTwoFail:
	...
	axarr[1].imshow(image)
	plt.title("Fail")
	plt.savefig('./'+img[1][0:(len(img[1])-4)]+'_FailCorner.png')
	plt.close("all")
